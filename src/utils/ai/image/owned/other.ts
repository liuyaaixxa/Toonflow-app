...
export default async (input: ImageConfig, config: AIConfig): Promise<string> => {
  ...
  // 已知的纯图片生成模型走 generateImage，其他模型（gpt、gemini、claude 等）走 generateText
  const pureImageModels = ["dall-e", "stable-diffusion", "midjourney", "wanx", "flux"];
  const isPureImageModel = pureImageModels.some((m) => model.toLowerCase().includes(m));
  if (!isPureImageModel) {
    let promptData;
