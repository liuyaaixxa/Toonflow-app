...
export default async (knex: Knex): Promise<void> => {
  ...
  // 添加索引以优化查询性能
  const addIndex = async (table: string, columns: string[], indexName: string) => {
    if (!(await knex.schema.hasTable(table))) return;
    try {
  ...
  await addIndex("t_image", ["assetsId"], "idx_image_assetsId");
  await addIndex("t_image", ["type"], "idx_image_type");
  await addIndex("t_assets", ["scriptId", "type"], "idx_assets_scriptId_type");
  await addIndex("t_assets", ["projectId", "type"], "idx_assets_projectId_type");
