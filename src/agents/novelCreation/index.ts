...
export default class NovelCreationAgent {
  ...
  private saveChapterTool = tool({
    title: "saveChapter",
    description: "保存单个章节到数据库。每完成一个章节的写作，必须调用此工具保存。",
    inputSchema: z.object({
      chapterIndex: z.number().describe("章节序号，从1开始"),
...
${context}`;
    ...
    const { fullStream } = await u.ai.text.stream(
      {
        ...
        messages: [{ role: "user", content: `请创作 ${params.chapterCount} 个章节的小说` }],
        maxStep: 50,
      },
      { model: "deepseek-chat" }
    );
    ...
    this.emit("complete", { message: "创作完成", count: params.chapterCount });
    return fullResponse;
  }
  ...
  async continueFrom(chapterId: number, prompt: string): Promise<string> {
    ...
    const maxChapter: any = await u.db("t_novel")
      .where({ projectId: this.projectId })
...
${context}`;
    ...
    const nextIndex = (maxChapter?.max || 0) + 1;

    const { fullStream } = await u.ai.text.stream(
      {
        system: systemPrompt,
        tools: { saveChapter: this.saveChapterTool },
        messages: [{ role: "user", content: `请续写第${nextIndex}章。${prompt}` }],
        maxStep: 20,
      },
      { model: "deepseek-chat" }
    );
